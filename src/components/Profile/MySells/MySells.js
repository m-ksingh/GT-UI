...
import MySalesList from './MySellsList';
import '../MyOrders/myorders.css';
import useToast from '../../../commons/ToastHook';
import _ from 'lodash';
import moment from 'moment';

const MySales = ({storeId}) => {
    ...
    const initUpcomingOrders = () => {
        try {
            ...
            ApiService.getMyActiveSells(userDetails.user.sid).then(
                response => {
    ...
    const initPastOrders = () => {
        spinner.show("Please wait...");
        let listOrdersByMonthly = [];
        ApiService.getMyCompletedSells(userDetails.user.sid).then(
            response => {
                response.data = _.map(response.data, o => {
                    if (o.orderDetailsTO.deliveryLocationType === 'SHERIFF_OFFICE') {
                        o.orderDetailsTO.sheriffOfficeLocation = JSON.parse(o.orderDetailsTO.sheriffOfficeLocation);
    ...
    return (<>
        <Tab.Container defaultActiveKey={key} onSelect={k => setKey(k)} className="mb-2">
            ...
            <Nav className="nav-tabs myac-tab-head" id="myTab" >
                <Nav.Item>
                    <Nav.Link eventKey={"upcoming-orders"} id="upcoming-orders">{"Active Orders"}</Nav.Link>
                </Nav.Item>
                <Nav.Item>
                    <Nav.Link eventKey={"past-orders"} id="past-orders">{"Completed Orders"}</Nav.Link>
                </Nav.Item>
            </Nav>
            <Tab.Content className="tab-body p-0">
                <Tab.Pane eventKey={"upcoming-orders"} >
                    {key === "upcoming-orders" && <>
                        <MySalesList {...{ lists: upcomingOrders, isDataLoaded, orderType: 'upcoming' }} /> </>}
                </Tab.Pane>
                <Tab.Pane eventKey={"past-orders"} >
                    {key === "past-orders" && <>
                        {isDataLoaded && <>  <MySalesList {...{ lists: pastOrders, isDataLoaded, orderType: 'past', onSuccess: () => initPastOrders() }} /></>}
                    </>}
                </Tab.Pane>
            </Tab.Content>
        </Tab.Container>
