...
const MySalesList = ({ lists, isDataLoaded, bidType="placed", onSuccess = () => {}}) => {
    ...
    return (<div className="">
        <div className="tab-pane">
            <div className="row">
                <div className="col">
                    {
                        isDataLoaded && myOrders.map((ordersByMonth) => {
                            return <div className="orders-by-month-block p-0">
                                        ...
                                        return <div className="myWishlistbox" key={index}>
                                            <div className="row border-bottom  pt-2 pb-2 pl-0 pr-0 mb-3">
                                                <div className="col pl-2">
                                                    ...
                                                    <span className="ordersDate orderId f12">Order #{order.orderDetailsTO.orderId}</span>
                                            ...
                                            <div className="row jcb">
                                                    ...
                                                    <div className="WishlistItem">
                                                        ...
                                                        <div className="prod-thumbnail-img" style={{backgroundImage: `url(${getMyImage(order.listingDetails)})`}}></div>
                                                            ...
                                                            <div className="media-body">
                                                                ...
                                                                <p className="my-order-by-width">Sold by: { order.listingDetails && order.listingDetails.fflStore ? ' ' + order.listingDetails.fflStore.name : order.listingDetails.appUser.firstName} </p>
                                                ...
                                                <div className="col-lg-2 col-3 pl-1 pr-1">
                                                ...
                                                <div className="col-lg-5 col-3 pl-3 pr-1 mobile-off">
                                                    <div className="location-block">
                                                        <p className="m-0 pr-1"><IcnLocation /></p>
                                                        <label className="order-location">
                                                            ...
                                                            {
                                                                order.orderDetailsTO.deliveryLocationType === 'OTHER_LOCATION' 
                                                                && <span>Other : {order.orderDetailsTO?.otherLocation?.formatted_address || order.orderDetailsTO?.otherLocation?.name || ""}</span>
                                                            }
                                                        </label>
                                                    </div>
                                                    {/* <div className="mt-1 ml-2">
                                                        { !order.returnable && order.orderListingDetailsStatus && <span className="order-status-btn statusBtn aic jcc">{order.orderListingDetailsStatus}</span> }
                                                        <span className="ml-2 orderDate"></span>
                                                    </div> */}
                                                </div>
                                            </div>
                                            <div className="row order-footer desktop-off">
                                                <div className="col-lg-12 location-block">
                                                <p className="m-0 pr-1"><IcnLocation /></p>
                                                ...
                                                {/* <div className="col-lg-12 mt-1">
                                                    {order.orderListingDetailsStatus && <button className="ml-2 btn btn-success btn-sm order-status-btn">{order.orderListingDetailsStatus}</button>}
                                                    <span className="ml-2 orderDate"></span>
                                                </div> */}
                                            </div>
                                            
                                            {/* order history details */}
                                            <div className="ohs-div">
                                                <div className="ohs-box-h-line"></div>
                                                <div className="ohs-box pl15 mb-3">
                                                   <div
                                                        className="p-rel"
                                                        aria-expanded={checkExpandData(order.orderHasListingSid)}
                                                        onClick={() => { onExpCollapse(order.orderHasListingSid); getOrderHistoryDetails(order.orderDetailsTO.sid) }}><div className="ohs-item aic pointer f10">
                                                            ...
                                                            <div className="ml10 ohs-date">{order.orderDetailsTO.orderLatestActivityDate ? `${moment(order.orderDetailsTO.orderLatestActivityDate).format('ll')} at ${moment(order.orderDetailsTO.orderLatestActivityDate).format('LT')}` : ""}</div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    </div>
                                                    {
                                                        ...
                                                        && order.orderHistoryDetails.map((r, i) => <Collapse in={checkExpandData(order.orderHasListingSid)} key={i}>
                                                            <div className="ohs-item aic f10">
                                                                ...
                                                                <div className="ml10 ohs-date">{moment(r.eventDate).format('ll')} at {moment(r.eventDate).format('LT')}</div>
                                                            </div>
                                                        </Collapse>)
                                                    }
                                                </div>
                                            ...
                                            <div className="row border-top justify-content-end pt-1 pb-1 rh-40">
                                                <div className="col-lg-2 col-4 pl-3 pr-1 mt-2">
                                                    <div className="mt-1 ml-2 float-right">
                                                    {
                                                        order
                                                        && order?.orderDetailsTO
                                                        && <button className="order-status-btn statusBtn aic jcc w100px cp-none">
